...
/* 20-10-2015 removed due to compiler error 
  ...
  if (is_abbrev(arg, "mtr")) {
    if ((victim = get_char_vis(ch, targ_name, FIND_CHAR_WORLD))) {
      if (IS_NPC(victim))  {
        ...
        if ( ( GET_MOB_VNUM(victim)/100 < GET_OLC_ZONE(ch)) 
          || ( ((GET_MOB_VNUM(victim)/100) - GET_OLC_ZONE(ch)) > GET_OLC_ZEXTENT(ch))) {
    ...
    if ((object = get_obj_vis(ch, targ_name))) {
      ...
      if ( ( GET_OBJ_VNUM(object)/100 < GET_OLC_ZONE(ch)) 
        || ( ((GET_OBJ_VNUM(object)/100) - GET_OLC_ZONE(ch)) > GET_OLC_ZEXTENT(ch))) {
        send_to_char("You do not have permission to edit this zone.\r\n", ch);
        return;
      }
    ...
    if (isdigit(*targ_name) && !strchr(targ_name, '.')) {
      if ((room = find_target_room(ch, targ_name)) != NOWHERE) {
        ...
        if ( ( GET_ROOM_VNUM(room)/100 < GET_OLC_ZONE(ch)) 
          || ( ((GET_ROOM_VNUM(room)/100) - GET_OLC_ZONE(ch)) > GET_OLC_ZEXTENT(ch))) {
          send_to_char("You do not have permission to edit this zone.\r\n", ch);
          return;
        }
...
{  
  ...
  if (!str_cmp(arg1, "room")) {
    ...
    if ( ( room->number/100 < GET_OLC_ZONE(ch)) || ( (room->number/100 - GET_OLC_ZONE(ch)) > GET_OLC_ZEXTENT(ch))) {
      send_to_char("You do not have permission to edit this zone.\r\n", ch);
      return;
    }
    ...
    if (victim) {
      ...
      else if ( ( GET_MOB_VNUM(victim)/100 < GET_OLC_ZONE(ch)) || ( ((GET_MOB_VNUM(victim)/100) - GET_OLC_ZONE(ch)) > GET_OLC_ZEXTENT(ch))) {
        send_to_char("You do not have permission to edit this zone.\r\n", ch);
        return;
      }
      ...
      if ( ( GET_OBJ_VNUM(object)/100 < GET_OLC_ZONE(ch)) || ( ((GET_OBJ_VNUM(object)/100) - GET_OLC_ZONE(ch)) > GET_OLC_ZEXTENT(ch))) {
        send_to_char("You do not have permission to edit this zone.\r\n", ch);
        return;
      }
      else if (!SCRIPT(object))
