...
struct char_data * tmp;
  ...
  if (ch == NULL || room < 0 || room > top_of_world)
    ...
    if (ch->next_in_room)
      log("SYSERR: next_in_room not NULL in char_to_room. Char not removed from previous location? (Ch: %s, Next: %s, OldRoomVnum: %d, NewRoomVnum: %d, File: %s, Line: %d)", ch->player.name, ch->next_in_room->player.name, GET_ROOM_VNUM(IN_ROOM(ch)), GET_ROOM_VNUM(room), __FILE__, __LINE__);

    for(tmp = world[room].people; tmp; tmp=tmp->next_in_room)
      if (tmp == ch) {
        log("SYSERR: Fatal error. Char already in people list of new room in char_to_room. (Ch: %s, OldRoomVnum: %d, NewRoomVnum: %d, File: %s, Line: %d)", ch->player.name, GET_ROOM_VNUM(IN_ROOM(ch)), GET_ROOM_VNUM(room), __FILE__, __LINE__);
	abort();
      }
