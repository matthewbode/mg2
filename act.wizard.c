...
#include "structs.h"
#include "attributes.h"     /* Added in due to compiler error */
#include "utils.h"
#include "comm.h"
...
int find_depth_in_room(struct char_data *ch, char *rawroomstr)
{
  int tmp;
  int depth = 0;
...
ACMD(do_goto)
{
  ...
  depth = find_depth_in_room(ch, argument);
...
void do_stat_room(struct char_data * ch)
{
  ...
  sprintf(buf, "Min depth: [%s%3d'%s], Max depth: [%s%3d'%s]\r\n",
  	rm->min_depth > 0 ? CCBLU(ch, C_NRM) : (rm->min_depth == 0 ? "" : CCCYN(ch, C_NRM)),
	rm->min_depth, CCNRM(ch, C_NRM),
  	rm->max_depth > 0 ? CCBLU(ch, C_NRM) : (rm->max_depth == 0 ? "" : CCCYN(ch, C_NRM)),
	rm->max_depth, CCNRM(ch, C_NRM));
  send_to_char(buf, ch);
...
void do_stat_object(struct char_data * ch, struct obj_data * j)
{
  ...
  if (IN_ROOM(j) == NOWHERE)
    ...
    sprintf(buf2, "%d [%s%d'%s]", GET_ROOM_VNUM(IN_ROOM(j)),
    	GET_ROOM_DEPTH(j) > 0 ? CCBLU(ch, C_NRM) : (GET_ROOM_DEPTH(j) == 0 ? "" : CCCYN(ch, C_NRM)),
    	GET_ROOM_DEPTH(j), CCNRM(ch, C_NRM));
    strcat(buf, buf2);
  }
  /*
...
void do_stat_character(struct char_data * ch, struct char_data * k)
{
  ...
  sprintf(buf2, " %s '%s'  IDNum: [%5ld], In room [%5d %s%d'%s]\r\n",
	  (!IS_NPC(k) ? "PC" : (!IS_MOB(k) ? "NPC" : "MOB")),
	  GET_NAME(k), GET_IDNUM(k), GET_ROOM_VNUM(IN_ROOM(k)),
	  GET_ROOM_DEPTH(k) > 0 ? CCBLU(ch, C_NRM) : (GET_ROOM_DEPTH(k) == 0 ? "" : CCCYN(ch, C_NRM)),
	  GET_ROOM_DEPTH(k), CCNRM(ch, C_NRM));
  ...
  if (!IS_NPC(k)) {
    ...
    sprintf(buf + strlen(buf),"\r\nPchannel: [%d]", GET_PRIVATE(k));
    if (GET_LEVEL(k) >= LVL_IMMORT) {
      sprintf(buf + strlen(buf), ", OLC: [%d]", GET_OLC_ZONE(k));
      sprintf(buf + strlen(buf), ", Extra OLC zones: [%d]", GET_OLC_ZEXTENT(k));
    }
    strcat(buf, "\r\n");
    send_to_char(buf, ch);
  }
...
ACMD(do_stat)
{
  ...
  if (!*buf1) {
    ...
    if ((object = get_object_in_equip_vis(ch, buf1, ch->equipment, &tmp)) != NULL)
      ...
      do_stat_character(ch, victim);
    else if ((object = get_obj_in_list_vis(ch, buf1, world[IN_ROOM(ch)].contents)) != NULL)
      do_stat_object(ch, object);
...
ACMD(do_purge)
{
  ...
  if (*buf) {			/* argument supplied. destroy single object
	...
	if (vict->desc) {
      ...
      extract_char(vict);
    } else if ((obj = get_obj_in_list_vis(ch, buf, world[IN_ROOM(ch)].contents)) != NULL) {
...
ACMD(do_wizutil)
{
  ...
  if (!*arg)
    ...
    send_to_char("You can't do that to someone as strong or stronger than you!\r\n", ch);
  else {
    switch (subcmd) {
    case SCMD_REROLL:
...
/* The set options available */
  struct set_struct {
   ...
   { "room_depth",	LVL_GRGOD,	BOTH,	NUMBER },
   { "private",		LVL_GOD,	PC,	NUMBER },
   { "\n", 0, BOTH, MISC }
  };
		...
		char *val_arg)
{
  ...
  case 51:
    two_arguments(val_arg, buf1, buf2);
    if (buf2)
      GET_OLC_ZONE(vict) = atoi(buf1);
    else {
      send_to_char("usage: set olc <zone> [range]\r\n", ch);
      return(0);
    }
    if (buf2)
      GET_OLC_ZEXTENT(vict) = atoi(buf2);
    break;

  case 52:
    GET_ROOM_DEPTH(vict) = value;
    if (value > world[IN_ROOM(vict)].max_depth || value < world[IN_ROOM(vict)].min_depth)
      send_to_char("Warning! New depth not in depth range of current room.\r\n", ch);
    break;
...
ACMD(do_zdelete)
{
  ...
  */
  ...
  for (i = 0; i <= top_of_zone_table; i++) {
    sprintf(buf2, "%s%d.wld", WLD_PREFIX, zone_table[i].number);
    if ((testfile = fopen(buf2, "r"))) {
      fclose(testfile);
      if (zone_table[i].number != j ) {
      	fprintf(index2, "%d.wld\n", zone_table[i].number);
      } else {
	sprintf(buf1, "%s%d.wld.%u", WLD_PREFIX, zone_table[i].number, (uint)time(NULL));
        if(rename(buf2, buf1))
          send_to_char("Warning: Could not rename .wld file. Please back up manually\r\n", ch);
      }
    } else {
      if (zone_table[i].number == j )
        send_to_char("WARNING: Deleted zone has no .wld file.\r\nEnsure that this is not a placeholder zone before reusing.\r\n", ch);
    }
  }
  fprintf(index2, "$\n");
  fclose(index2);
  ...
  for (i = 0; i <= top_of_zone_table; i++) {
    sprintf(buf2, "%s%d.mob", MOB_PREFIX, zone_table[i].number);
    if ((testfile = fopen(buf2, "r"))) {
      fclose(testfile);
      if (zone_table[i].number != j ) {
      	fprintf(index2, "%d.mob\n", zone_table[i].number);
      } else {
	sprintf(buf1, "%s%d.mob.%u", MOB_PREFIX, zone_table[i].number, (uint)time(NULL));
        if(rename(buf2, buf1))
          send_to_char("Warning: Could not rename .mob file. Please back up manually\r\n", ch);
      }
    } else {
      if (zone_table[i].number == j )
        send_to_char("Info: Deleted zone has no .mob file.\r\n", ch);
    }
  }
  fprintf(index2, "$\n");
  fclose(index2);
  ...
  for (i = 0; i <= top_of_zone_table; i++) {
    sprintf(buf2, "%s%d.obj", OBJ_PREFIX, zone_table[i].number);
    if ((testfile = fopen(buf2, "r"))) {
      fclose(testfile);
      if (zone_table[i].number != j ) {
      	fprintf(index2, "%d.obj\n", zone_table[i].number);
      } else {
	sprintf(buf1, "%s%d.obj.%u", OBJ_PREFIX, zone_table[i].number, (uint)time(NULL));
        if (rename(buf2, buf1))
          send_to_char("Warning: Could not rename .obj file. Please back up manually\r\n", ch);
      }
    } else {
      if (zone_table[i].number == j )
        send_to_char("Info: Deleted zone has no .obj file.\r\n", ch);
    }
  }
  fprintf(index2, "$\n");
  fclose(index2);
  ...
  for (i = 0; i <= top_of_zone_table; i++) {
    sprintf(buf2, "%s%d.shp", SHP_PREFIX, zone_table[i].number);
    if ((testfile = fopen(buf2, "r"))) {
      fclose(testfile);
      if (zone_table[i].number != j ) {
      	fprintf(index2, "%d.shp\n", zone_table[i].number);
      } else {
	sprintf(buf1, "%s%d.shp.%u", SHP_PREFIX, zone_table[i].number, (uint)time(NULL));
        if (rename(buf2, buf1))
          send_to_char("Warning: Could not rename .shp file. Please back up manually\r\n", ch);
      }
    } else {
      if (zone_table[i].number == j )
        send_to_char("Info: Deleted zone has no .shp file.\r\n", ch);
    }
  }
  fprintf(index2, "$\n");
  fclose(index2);
  ...
  /* Reindex TRG files */
  sprintf(buf2, "%s%s", TRG_PREFIX, INDEX_FILE);
  if (!(index2 = fopen(buf2, "w"))) {
    log("SYSERR: Error opening an index file in zdelete");
    send_to_char("Error. Please Delete zone manually then reboot.\r\n", ch);
    return;
  ...
  for (i = 0; i <= top_of_zone_table; i++) {
    sprintf(buf2, "%s%d.trg", TRG_PREFIX, zone_table[i].number);
    if ((testfile = fopen(buf2, "r"))) {
      fclose(testfile);
      if (zone_table[i].number != j ) {
      	fprintf(index2, "%d.trg\n", zone_table[i].number);
      } else {
	sprintf(buf1, "%s%d.trg.%u", TRG_PREFIX, zone_table[i].number, (uint)time(NULL));
        if (rename(buf2, buf1))
          send_to_char("Warning: Could not rename .trg file. Please back up manually\r\n", ch);
      }
    } else {
      if (zone_table[i].number == j )
        send_to_char("Info: Deleted zone has no .trg file.\r\n", ch);
    }
  }
  fprintf(index2, "$\n");
  fclose(index2);
  ...
  for (i = 0; i <= top_of_zone_table; i++) {
    sprintf(buf2, "%s%d.zon", ZON_PREFIX, zone_table[i].number);
    if ((testfile = fopen(buf2, "r"))) {
      fclose(testfile);
      if (zone_table[i].number != j ) {
      	fprintf(index2, "%d.zon\n", zone_table[i].number);
      } else {
	sprintf(buf1, "%s%d.zon.%u", ZON_PREFIX, zone_table[i].number, (uint)time(NULL));
        if (rename(buf2, buf1))
          send_to_char("Warning: Could not rename .zon file. Please back up manually\r\n", ch);
      }
    } else {
      if (zone_table[i].number == j )
        send_to_char("WARNING: Deleted zone has no .zon file.\r\nThis is very odd. Please investigate.\r\n", ch);
    }
  }
  fprintf(index2, "$\n");
  fclose(index2);
...
ACMD(do_copyto)
{
  ...
  if (!*buf2) {
    sprintf(buf, "Usage: %s <room number>\r\n", (subcmd == SCMD_RCLONE) ? "rclone" : "rcopy" );
    send_to_char(buf, ch);
    return;
  }
...
/* Main stuff */
  if (world[IN_ROOM(ch)].description) {
    ...
    if (subcmd == SCMD_RCLONE) {
      if (world[IN_ROOM(ch)].name)
        world[rroom].name = str_dup(world[IN_ROOM(ch)].name);
      if (world[IN_ROOM(ch)].room_flags)
        world[rroom].room_flags = world[IN_ROOM(ch)].room_flags;
      if (world[IN_ROOM(ch)].sector_type)
    ...
    send_to_char("This room has no description so why copy it?!\r\n", ch);
  } 
}
...
/* Main stuff */
  ...
  add_to_save_list((iroom/100), SL_WLD);
  add_to_save_list((world[IN_ROOM(ch)].number/100), SL_WLD);
...
ACMD(do_bolt)
{
  ...
  one_argument(argument, buf);
  if (!*buf)
    send_to_char("Who do you wish to bolt?\r\n", ch);

  else if (!(vict = get_char_vis(ch, buf, FIND_CHAR_WORLD)))
    send_to_char(NOPERSON, ch);

  else if (IS_NPC(ch))   
    send_to_char("Oh no you don't! No bolting mobiles!\r\n", ch);
    ...
    send_to_char("Now that's just plain foolishness!\r\n", ch);
	  ...
	  send_to_char("You cant bolt someone stronger than you!\r\n", ch);
    ...
    GET_HIT(vict) = (GET_HIT(vict) - 50); 
    GET_MANA(vict) = (GET_MANA(vict) - 50);
    GET_MOVE(vict) = (GET_MOVE(vict) - 50);
    ...
    sprintf(buf, "(GC) %s bolted %s for %dH, %dM, %dV (stats - 50)",
    GET_NAME(ch), GET_NAME(vict), NHIT, NMANA, NMOVE);
    mudlog(buf, BRF, MAX(LVL_IMMORT, GET_INVIS_LEV(ch)), TRUE);

    update_pos(vict); /* Change this to whatever you want */
    send_to_char("You are frazzled by a lightning bolt from the heavens!", vict);
    send_to_char("You send down a lightning bolt...", ch);
  }
}
...
ACMD(do_anvil)
{
  ...
  one_argument(argument, buf);
  if (!*buf)
    send_to_char("Who do you wish to anvil?\r\n", ch);

  else if (!(vict = get_char_vis(ch, buf, FIND_CHAR_WORLD)))
    send_to_char(NOPERSON, ch);

  else if (IS_NPC(ch))   
    send_to_char("Oh no you don't! No using an anvil on mobiles!\r\n", ch);
    ...
    send_to_char("Now that's just plain foolishness!\r\n", ch);
        ...
        if(GET_LEVEL(vict) > GET_LEVEL(ch)){
	  send_to_char("You cant anvil someone stronger than you!\r\n", ch);

	  send_to_char("You laugh as you flick aside the anvil coming toward you.\r\n", vict);
	  vict = ch;}
    ...
    GET_MANA(vict) = (GET_MANA(vict) - 100);
    GET_MOVE(vict) = (GET_MOVE(vict) - 100);
    ...
    /* Then display and syslog it */
    sprintf(buf, "(GC) %s anvilled %s for %dH, %dM, %dV (stats - 100)",
    GET_NAME(ch), GET_NAME(vict), NHIT, NMANA, NMOVE);
    mudlog(buf, BRF, MAX(LVL_IMMORT, GET_INVIS_LEV(ch)), TRUE);

    update_pos(vict); /* Change this to whatever you want */
    send_to_char("An anvil falls from the sky and lands on your head!", vict);
    send_to_char("Bombs away!!!", ch);
  }
}
...
char *name = arg, *timetolag = buf2; 
 ...
 two_arguments(argument, name, timetolag);
  ...
  if (!*timetolag || !*name)
  {
   send_to_char("Usage: <character> <lag time (seconds)>\r\n", ch);
   return;
  }
...
if (strcmp(GET_NAME(vict), "moradin") == 0)
{
