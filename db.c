...
void parse_room(FILE * fl, int virtual_nr)
{
  ...
  int t[10], i;
  sh_int min_depth = 0, max_depth = 0;
  ...
  if (sscanf(line, " %d %s %d %hd %hd ", t, flags, t + 2, &max_depth, &min_depth) < 3) {
  ...
  world[room_nr].min_depth = min_depth;
  world[room_nr].max_depth = max_depth;
...
char *parse_object(FILE * obj_f, int nr)
{
  ...
  obj_proto[i].room_data.in_room = NOWHERE;
  ...
  if((id = find_name(name)) < 0) {
	...
	if(!strcmp(tag, "Name"))
	  GET_PC_NAME(ch) = str_dup(line);
          /* 20-10-15 changed GET_NAME to GET_PC_NAME due to comp error */
	break;
...
void save_char(struct char_data * ch, room_rnum load_room)
  ...
  */
...

  {
    ...
    if(GET_OLC_ZONE(ch) != NOTHING)
      fbprintf(fl, "Zone: %d\n", GET_OLC_ZONE(ch));
    if(GET_OLC_ZEXTENT(ch) > 0)
      fbprintf(fl, "Zext: %d\n", GET_OLC_ZEXTENT(ch));
  ...
  do {
    ...
    if ((point = strchr(tmp, '~')) != NULL) {
      *point = '\0';
      ...
      if(!ISNEWL(*point))
        log("SYSERR: fread_string: line too long");
      else {
        *(point++) = '\r';
        *(point++) = '\n';
        *point = '\0';
...
void free_char(struct char_data * ch)
{
  ...
  if (!IS_NPC(ch) || (IS_NPC(ch) && GET_MOB_RNUM(ch) == -1)) {
    ...
    if (ch->player.afk_message);
      free(ch->player.afk_message);
