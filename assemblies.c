...
void assemblyBootAssemblies( void )
{
  ...
  while( !feof( pFile ) )
  {
    ...
    {
      if( sscanf( szLine, "#%ld %ld %s", &lVnum, &failVnum, szType ) != 3 )
      {
	...
	lVnum = NOTHING;
      ...
      else if( !assemblyCreate( lVnum, failVnum, iType ) )
      {
...
void assemblySaveAssemblies( void )
{
  ...
  for( i = 0; i < g_lNumAssemblies; i++)
  {
    ...
    fprintf( pFile, "Vnum                #%ld %ld %s\n",
             pAssembly->lVnum, pAssembly->failVnum, szType );
...
void assemblyListToChar( struct char_data *pCharacter )
{
  ...
  for( i = 0; i < g_lNumAssemblies; i++ )
  {
    ...
    else
    {
      ...
      sprintf( szBuffer, "[%5ld] %s (%s) [%5ld]\r\n", g_pAssemblyTable[i].lVnum,
           obj_proto[lRnum].short_description, szAssmType, g_pAssemblyTable[i].failVnum);
...
bool assemblyCreate( long lVnum, long failVnum, int iAssembledType )
{
  long		lBottom = 0;
  long		lMiddle = 0;
  long		lTop = 0;
  ...
  g_pAssemblyTable[ lMiddle ].failVnum = failVnum;
...
/* This is a copy of assemblyFindAssembly, but uses the fail info instead.
 * There is probably a better way to do this, but it works. */
long assemblyFailAssembly( const char *pszAssemblyName )
{
  long		i = 0;
  long		lRnum = NOTHING;

  if( g_pAssemblyTable == NULL ) {
     log("if( g_pAssemblyTable == NULL )");
    return (-1);
    }
  else if( pszAssemblyName == NULL || *pszAssemblyName == '\0' ) {
    log("if( pszAssemblyName == NULL  *pszAssemblyName ==  )");
    return (-1);
   }
  for( i = 0; i < g_lNumAssemblies; i++ )
  {
    if( (lRnum = real_object( g_pAssemblyTable[ i ].lVnum )) < 0 )
      log( "SYSERR: assemblyFailAssembly(): Invalid vnum #%ld in assembly table.",
             g_pAssemblyTable[i].lVnum);
    else if (isname(pszAssemblyName, obj_proto[ lRnum ].name ))
      return (g_pAssemblyTable[i].failVnum);
  }
  return (-1);
}
