...
int do_simple_move(struct char_data *ch, int dir, int need_specials_check)
{
  ...
  if ((SECT(IN_ROOM(ch)) == SECT_WATER_NOSWIM) ||
      ...
      (SECT(IN_ROOM(ch)) == SECT_SEA) ||
      (SECT(EXIT(ch, dir)->to_room) == SECT_SEA)) {
    if (!has_boat(ch)) {
  ...
  if (GET_ROOM_DEPTH(ch) < world[EXIT(ch, dir)->to_room].min_depth) {
    sprintf(buf2, "You need to go down about %d feet to go there.\r\n",
      world[EXIT(ch, dir)->to_room].min_depth - GET_ROOM_DEPTH(ch));
    send_to_char(buf2, ch);
    return 0;
  }
  if (GET_ROOM_DEPTH(ch) > world[EXIT(ch, dir)->to_room].max_depth) {
    sprintf(buf2, "You need to go up about %d feet to go there.\r\n",
      GET_ROOM_DEPTH(ch) - world[EXIT(ch, dir)->to_room].max_depth);
    send_to_char(buf2, ch);
    return 0;
  }
...
ACMD(do_depth)
{
  ...
  case SCMD_FLY:
    ...
    case 'D':
      ...
      break;
    default:
      send_to_char("Which way do you want to fly? Up or down?\r\n", ch);
      return;
    }

    GET_ROOM_DEPTH(ch) = MAX(world[IN_ROOM(ch)].min_depth, MIN(world[IN_ROOM(ch)].max_depth, GET_ROOM_DEPTH(ch) + (GET_LEVEL(ch) > LVL_IMMORT && atoi(buf2) ? atoi(buf2) : 10) * direction));
    look_at_room(ch, 0);

    break;
  case SCMD_SWIM:
    send_to_char("Sorry, swim not yet implemented. You can fly though...\r\n", ch);
  }
  return;
}
