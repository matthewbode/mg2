...
ACMD(do_oasis)
{
  ...
  if (!*buf1) {		/* No argument given. */
    ...
    if (str_cmp("save", buf1) == 0) {
      ...
      if ((number = (*buf2 ? atoi(buf2) : ((GET_OLC_ZONE(ch) && GET_OLC_ZEXTENT(ch)==0) ? GET_OLC_ZONE(ch) : -1)) * 100) < 0 ) {
	...
	send_to_char("Specify a new zone number.\r\n", ch);
      ...
      send_to_char("You don't know what you are doing, do you!\r\n", ch);
      return;
    }
  }
  ...
  if (GET_LEVEL(ch) < LVL_IMPL) {
    if (subcmd == SCMD_OASIS_HEDIT && GET_OLC_ZONE(ch) != HEDIT_PERMISSION) {
      send_to_char("You do not have permssion to edit help entries.\r\n", ch);
      free(d->olc);
      d->olc = NULL;
      return;
    } else
    if (((zone_table[OLC_ZNUM(d)].number < GET_OLC_ZONE(ch)) || ((zone_table[OLC_ZNUM(d)].number - GET_OLC_ZONE(ch)) > GET_OLC_ZEXTENT(ch))) && subcmd != SCMD_OASIS_HEDIT) {
  ...
  if (save) {
    ...
    switch (subcmd) {
      ...
      case SCMD_OASIS_TRIGEDIT: trigedit_save(d); break;
    }
    free(d->olc);
    d->olc = NULL;
    return;
...
/*  int i, found = FALSE, rnum, zone, zn; */
  ...
  zone = (world[IN_ROOM(ch)].number / 100);
