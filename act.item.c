...
char *auctioneer[AUC_BID + 1] = {
	...
	"`mThe auctioneer auctions, '$n puts $p up for sale at %d coins.'`n",
	"`mThe auctioneer auctions, '$p at %d coins going once!'`n",
	"`mThe auctioneer auctions, '$p at %d coins going twice!'`n",
	"`mThe auctioneer auctions, 'Last call: $p being sold by $n for %d coins.'`n",
	"`mThe auctioneer auctions, 'Unfortunately $p is unsold, returning it to $n.'`n",
	"`mThe auctioneer auctions, 'SOLD! $p to $n for %d coins!'`n",
	"`mThe auctioneer auctions, 'Sorry, $n has cancelled the auction.'`n",
	"`mThe auctioneer auctions, 'Sorry, $n has left us, the auction can't go on.'`n",
	"`mThe auctioneer auctions, 'Sorry, $p has been confiscated, shame on you $n.'`n",
	"`mThe auctioneer tells you, '$n is selling $p for %d gold.'`n",
	"`mThe auctioneer auctions, '$n bids %d coins on $p.'`n"
};
/* local functions */
...
void start_auction(struct char_data * ch, struct obj_data * obj, int bid)
{
	...
	sprintf(buf, "%s magically flies away from your hands to be auctioned!\r\n", obj_selling->short_description);
	CAP(buf);
...
void check_auction(void)
{
	...
	case AUC_LAST_CALL:
	{
		...
		if (ch_buying == NULL) {
			...
			sprintf(buf, "%s flies out from the sky and into your hands.\r\n", obj_selling->short_description);
			CAP(buf);
			send_to_char(buf, ch_selling);
		...
		else
		{
			...
			sprintf(buf, "%s flies out from the sky and into your hands, what a steal!\r\n", obj_selling->short_description);
			CAP(buf);
...
void auc_send_to_all(char *messg, bool buyer)
{
  ...
  for (i = descriptor_list; i; i = i->next)
  {
	...
	send_to_char((i->character), i->character);
...
ACMD(do_assemble)
{
  long	 lVnum = NOTHING, fail_lVnum = NOTHING;
  int    pfail;
  struct obj_data *failObject = NULL;
  ...
  if (*argument == '\0') {
    ...
    send_to_char("You haven't got all the things you need.\r\n", ch);
    return;
  } else if ((fail_lVnum = assemblyFailAssembly(argument)) < 0) {
    send_to_char("Failed assembly code is corrupt, please report to an IMP", ch);
    return;
  }

  /* Just made it random for now, would be better to check dex/agi if fletching etc, or skill 
   * for cooking, etc. */

  pfail = number(1, 101);
  if (pfail > 50){
	 if ((lVnum = assemblyFailAssembly(argument)) < 0) {
                act("With shaking hands, $n makes a misserable attemt at something.\r\n",
                     FALSE, ch, 0, 0, TO_ROOM);
		sprintf(buf, "You completely fail your attempt to %s %s %s.\r\n",
                     CMD_NAME, AN(argument), argument);
		send_to_char(buf, ch);
		return;
	 } else {
     /* Can make a list of possible oops messages to call from here instead ... */
     send_to_char("Hrm... not quite what you had in mind.\r\n", ch);
     act("With shaking hands, $n makes a misserable attemt at something.\r\n", FALSE, ch, 0, 0, TO_ROOM);

     if ((failObject = read_object(assemblyFailAssembly(argument), VIRTUAL)) != NULL)
       obj_to_char(failObject, ch);
     return;
     }
   }
...
int can_take_obj(struct char_data * ch, struct obj_data * obj)
{
  if (!IN_MELEE_RANGE(ch, obj)) {
    act("$p: you are too far away to get that item.", FALSE, ch, obj, 0, TO_CHAR);
    return (0);
		            ...
		            byte mode, room_rnum RDR)
{
  ...
  if (amount <= 0)
	...
	GET_ROOM_DEPTH(obj) = GET_ROOM_DEPTH(ch);
      }
    } else {
...
struct char_data *give_find_vict(struct char_data * ch, char *arg)
{
  ...
  if (!*arg) {
    ...
    return (NULL);
  } else if (!IN_MELEE_RANGE(ch, vict)) {
    send_to_char("That person is too far away to give anything too.\r\n", ch);
    return (NULL);
  } else
    return (vict);
}
