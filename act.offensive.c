...
ACMD(do_hit)
{
  ...
  if (!*arg)
    ...
    if ((GET_POS(ch) == POS_STANDING) && (vict != FIGHTING(ch)) && (char_within_range_of_vict(ch, vict))) {
      hit(ch, vict, TYPE_UNDEFINED);
      WAIT_STATE(ch, PULSE_VIOLENCE + 2);
    } else if (!char_within_range_of_vict(ch, vict))
      send_to_char("You're not within range!\r\n", ch);
    else
...
ACMD(do_backstab)
{
  ...
  if (!IN_MELEE_RANGE(ch, vict)) {
    send_to_char("You aren't close enough to backstab!\r\n", ch);
    return;
  }
...
ACMD(do_flee)
{
  ...
  if (GET_POS(ch) >= POS_STANDING) {
    send_to_char("This can only be done in the heat of battle.\r\n", ch);
    return;
  }
...
ACMD(do_bash)
{
  ...
  if (!IN_MELEE_RANGE(ch, vict)) {
    send_to_char("You aren't close enough to bash!\r\n", ch);
    return;
  }
