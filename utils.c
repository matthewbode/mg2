...
char *strpaste(char *str1, char *str2, char *joiner) {
	...
	while((rp - ret) < MAX_STRING_LENGTH && (*sp1 || *sp2)) {
 		/* Copy line from str1 */
		while((rp - ret) < MAX_STRING_LENGTH && *sp1 && !iseol(*sp1))
			*rp++ = *sp1++;
		/* Eat the newline */
		if(*sp1) {
			if(sp1[1] && sp1[1]!=sp1[0] && iseol(sp1[1]))
				sp1++;
			sp1++;
		}
		...
		rp += jlen;
 		...
 		/* Copy line from str2 */
		while((rp - ret) < MAX_STRING_LENGTH && *sp2 && !iseol(*sp2))
			*rp++ = *sp2++;
		/* Eat the newline */
		if(*sp2) {
			if(sp2[1] && sp2[1]!=sp2[0] && iseol(sp2[1]))
				sp2++;
			sp2++;
		}
