 ...
 if (!isdigit(*buf)) {
    if (strn_cmp("new", buf, 3) == 0) {
         if(!isdigit(*buf2))
            ...
            assemblyCreate(atoi(buf2), atoi(buf2), 0);
...
void assedit_setup(struct descriptor_data *d, int number)
{
    ...
    if( (pOldAssembly = assemblyGetAssemblyPtr( number )) == NULL ) {
      ...
      return;
        ...
        OLC_ASSEDIT(d)->failVnum = pOldAssembly->failVnum;
 ...
 switch (OLC_MODE(d)) {
   ...
   case ASSEDIT_MAIN_MENU:
     ...
     case 'Q':                /* do the quit stuff */
       ...
       assemblyCreate(OLC_ASSEDIT(d)->lVnum, OLC_ASSEDIT(d)->failVnum, OLC_ASSEDIT(d)->uchAssemblyType);
       for( i = 0; i < OLC_ASSEDIT(d)->lNumComponents; i++) {
...
send_to_char(buf, d->character);
     ...
     case 'f':    /* Specify the item to make if we fail making the intended */
     case 'F':
       send_to_char("\r\nWhat is the vnum of the object to make upon failing? ", d->character);
       OLC_MODE(d) = ASSEDIT_SPEC_FAILOBJ;
     break;
 ...
 case ASSEDIT_SPEC_FAILOBJ:
   if (isdigit(*arg)){
     pos = atoi(arg);
     OLC_ASSEDIT(d)->failVnum = pos;
   }
   assedit_disp_menu(d);
   break;
