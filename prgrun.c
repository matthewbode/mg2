...
void process_program_output() {
  ...
  if ((len==-1) && (errno==EAGAIN)) { /* No datas are available now */
    if (time(0)<start_time+program_queue[0].timeout)
      ...
      sprintf(buf, "Killing %s because of timeout.\r\n",
	      program_queue[0].name);
  } else if (len < 0) {	 /* Error with read or timeout */
    sprintf(buf, "Error with reading from %s, killed.",
	    program_queue[0].name);
  } else if (len == 0) {  /* EOF readed  */
    if (had_output)
      buf[0]=0;
    else
      sprintf(buf, "No output from %s.\r\n", program_queue[0].name);    
  };
...
/* Will boot the mud Nexus */

ACMD(do_nexus) {
  struct program_info nexus;
 
  nexus.args[0]=strdup("pine");
  nexus.args[1]=NULL;
  nexus.input=NULL;
  nexus.timeout=999;
  nexus.name=strdup("nexus");
  add_program(nexus, ch); 
}
