...
ACMD(do_steal)
{
  ...
  if (!IN_MELEE_RANGE(ch, vict)) {
    send_to_char("You are too far away to steal anything!\r\n", ch);
    return;
  }
...
ACMD(do_report)
{
  ...
  case SCMD_FLEX:
    ...
    send_to_char("You flex, but nobody seems to be impressed.\r\n", ch);
  default:
    return;
  }
}
...
ACMD(do_gen_tog)
{
  ...
  const char *tog_messages[][2] = {
    ...
    {"[`mAuction`n] Channel enabled.\r\n",
    "[`mAuction`n] Channel disabled.\r\n"},
    {"You can now hear shouts.\r\n",
    "You are now deaf to shouts.\r\n"},
    {"[`gRoleplay`n] Channel enabled.\r\n",
    "[`gRoleplay`n] Channel disabled.\r\n"},
    {"[`gCongrats`n] Channel enabled.\r\n",
    "[`gCongrats`n] Channel disabled.\r\n"},
    ...
    {"BuildWalk disabled.\r\n",
    "BuildWalk enabled.\r\n"},
    {"[`yOOC`n] Channel enabled.\r\n",
    "[`yOOC`n] Channel disabled.\r\n"},
    {"[`mNewbie`n] Channel enabled.\r\n",
    "[`mNewbie`n] Channel disabled.\r\n"}
  };
  ...
  case SCMD_DEAF:
    ...
    break;
  case SCMD_NOROLE:
    result = PRF_TOG_CHK(ch, PRF_NOROLE);
    break;
  case SCMD_NOGRATZ:
  ...
  case SCMD_NOOOC:
    ...
    break;
  case SCMD_NONEW:
    result = PRF_TOG_CHK(ch, PRF_NONEW);
    break;
  default:
