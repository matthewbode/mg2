...
int buildwalk(struct char_data *ch, int dir) {
  ...
  if (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_BUILDWALK) &&
   GET_LEVEL(ch) >= LVL_BUILDER) {
    ...
    if ((zone_table[world[IN_ROOM(ch)].zone].number < GET_OLC_ZONE(ch)
     || zone_table[world[IN_ROOM(ch)].zone].number - GET_OLC_ZONE(ch) > GET_OLC_ZEXTENT(ch))
