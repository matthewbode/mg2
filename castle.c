...
SPECIAL(king_welmar)
{
  ...
  if (!move) {
    if (time_info.hours == 8 && IN_ROOM(ch) == R_ROOM(Z_KINGS_C, 51)) {
      move = TRUE;
      path = throne_path;
      index = 0;
    } else if (time_info.hours == 21 && IN_ROOM(ch) == R_ROOM(Z_KINGS_C, 17)) {
      move = TRUE;
      path = bedroom_path;
      index = 0;
    } else if (time_info.hours == 12 && IN_ROOM(ch) == R_ROOM(Z_KINGS_C, 17)) {
      move = TRUE;
      path = monolog_path;
      index = 0;
    }
