			...
			int mode)
{
  ...
  *buf = '\0';
  if ((mode == 0) && object->description) {
    ...
    rdd = room_depth_difference(ch, object);
    if (rdd >= 10 ) {
      sprintf(buf2, " [at %d']", GET_ROOM_DEPTH(object));
      strcat(buf, buf2);
    }
  }
...
void list_one_char(struct char_data * i, struct char_data * ch)
{
  ...
  if (IS_NPC(i) && i->player.long_descr && GET_POS(i) == GET_DEFAULT_POS(i)) {
    ...
    rdd = room_depth_difference(ch, i);
    if (rdd >= 10) {
      sprintf(buf2, " [at %d']\r\n", GET_ROOM_DEPTH(i));
      strcpy(&buf[strlen(buf)-2], buf2);
    }
    send_to_char(buf, ch);
  ...
  if (!IS_NPC(i)) {
    ...
    if (PRF_FLAGGED(i, PRF_AFK))
      strcat(buf, " (afk)");
    if (PLR_FLAGGED(i, PRF_BUILDWALK))
  ...
  rdd = room_depth_difference(ch, i);
  if (rdd >= 10)
    sprintf(buf, "%s [at %d']", buf, GET_ROOM_DEPTH(i));
...
void look_at_room(struct char_data * ch, int ignore_brief)
{
  ...
  if (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_ROOMFLAGS)) {
    sprintbit(ROOM_FLAGS(IN_ROOM(ch)), room_bits, buf);
    sprintf(buf2, "[%5d] %s [ %s]", GET_ROOM_VNUM(IN_ROOM(ch)),
	    world[IN_ROOM(ch)].name, buf);
    send_to_char(buf2, ch);
  } else
    send_to_char(world[IN_ROOM(ch)].name, ch);

  send_to_char(CCNRM(ch, C_NRM), ch);

  if ((world[IN_ROOM(ch)].max_depth > 0 && GET_ROOM_DEPTH(ch) == world[IN_ROOM(ch)].max_depth) || GET_ROOM_DEPTH(ch) != world[IN_ROOM(ch)].max_depth) {
    sprintf(buf2, " %s[at %d']%s",
    	GET_ROOM_DEPTH(ch) > 0 ? CCBLU(ch, C_NRM) : (GET_ROOM_DEPTH(ch) == 0 ? "" : CCCYN(ch, C_NRM)),
    	GET_ROOM_DEPTH(ch),
    	CCNRM(ch, C_NRM));
    send_to_char(buf2, ch);
  }
  ...
  if ((!IS_NPC(ch) && !PRF_FLAGGED(ch, PRF_BRIEF)) || ignore_brief ||
    ...
    if (GET_ROOM_DEPTH(ch) == world[IN_ROOM(ch)].max_depth)
      str_and_map(world[IN_ROOM(ch)].description, ch);
    else if (GET_ROOM_DEPTH(ch) > 0) { /* In water or underground */
      if ((desc = find_exdesc(DEPTH_DESC_DEPTH, world[IN_ROOM(ch)].ex_description)) == NULL)
        desc = (world[IN_ROOM(ch)].sector_type == SECT_SEA) ?
        "Blub, blub, blub...\\\\There's water everywhere, and lots of tiny fishes.\r\n" :
	"Oh dear! You seem to have been buried alive.\r\n";
      str_and_map(desc, ch);
    } else if (GET_ROOM_DEPTH(ch) == 0) {
      if ((desc = find_exdesc(DEPTH_DESC_SURFACE, world[IN_ROOM(ch)].ex_description)) == NULL)
        desc = (world[IN_ROOM(ch)].sector_type == SECT_SEA) ?
        "The rippling surface of the sea stretches away on all sides.\r\n" :
        "Your feet are on the ground, but you get the feeling that there is something more interesting going on below you.\r\n";
      str_and_map(desc, ch);
    } else {
      if ((desc = find_exdesc(DEPTH_DESC_FLY, world[IN_ROOM(ch)].ex_description)) == NULL)
        desc = "You are flying.\r\n";
      str_and_map(desc, ch);
    }
  }
...
void look_at_target(struct char_data * ch, char *arg)
{
  ...
  if(strncasecmp(arg, DEPTH_DESC_PREFIX, strlen(DEPTH_DESC_PREFIX)))
...
strcat(buf, "\r\n");
sprintf(buf + strlen(buf),       "(')Name [%-14s]                                      (')\r\n",
GET_NAME(ch));
strcat(buf,                      "|/|-----------------------------------------------------------|=|\r\n");
sprintf(buf + strlen(buf),       "|=|STR[%2d][%3d]   |                    |                      |/|\r\n",
GET_STR(ch), GET_ADD(ch));
switch (GET_LEVEL(ch)) {
	case LVL_IMMORT:
    sprintf(buf + strlen(buf),   "|/|INT[%2d]        | Level [ Immortal ] | Move [%6d][%6d]|=|\r\n",
	GET_INT(ch), GET_MOVE(ch), GET_MAX_MOVE(ch));
    break;
	case LVL_GOD:
    sprintf(buf + strlen(buf),   "|/|INT[%2d]        | Level [   God    ] | Move [%6d][%6d]|=|\r\n",
	GET_INT(ch), GET_MOVE(ch), GET_MAX_MOVE(ch));
    break;
    case LVL_GRGOD:
    sprintf(buf + strlen(buf),   "|/|INT[%2d]        | Level[Greater-God] | Move [%6d][%6d]|=|\r\n",
	GET_INT(ch), GET_MOVE(ch), GET_MAX_MOVE(ch));
    break;
	case LVL_IMPL:
    sprintf(buf + strlen(buf),   "|/|INT[%2d]        | Level[Implementor] | Move [%6d][%6d]|=|\r\n",
	GET_INT(ch), GET_MOVE(ch), GET_MAX_MOVE(ch));
	break;
    default:
	sprintf(buf + strlen(buf), "|/|INT[%2d]        | Level[%3d]         | Move [%6d][%6d]|=|\r\n",
	GET_INT(ch), GET_LEVEL(ch), GET_MOVE(ch), GET_MAX_MOVE(ch));
    break;
}
sprintf(buf + strlen(buf),       "|=|WIS[%2d]        | Exp  [%10d]  | Life [%6d][%6d]|/|\r\n",
GET_WIS(ch), GET_EXP(ch), GET_HIT(ch), GET_MAX_HIT(ch));
if (GET_LEVEL(ch) >= LVL_IMMORT)
	sprintf(buf + strlen(buf), "|/|DEX[%2d]        | Olc  [%4d][%4d]  | Mana [%6d][%6d]|=|\r\n",
	GET_DEX(ch), GET_OLC_ZONE(ch), GET_OLC_ZEXTENT(ch), GET_MANA(ch), 
GET_MAX_MANA(ch));
  else
	sprintf(buf + strlen(buf), "|/|DEX[%2d]        | Need [%10d]  | Mana [%6d][%6d]|=|\r\n",
	GET_DEX(ch), level_exp(GET_CLASS(ch), GET_LEVEL(ch) + 1) - GET_EXP(ch), GET_MANA(ch), GET_MAX_MANA(ch));
sprintf(buf + strlen(buf),       "|=|CON[%2d]        |___________________________________________|/|\r\n",
GET_CON(ch));
sprintf(buf + strlen(buf),       "|/|CHA[%2d]        | Gold [%8d]  | Armour [%5d]         |=|\r\n",
GET_CHA(ch), GET_GOLD(ch), GET_AC(ch));

  playing_time = *real_time_passed((time(0) - ch->player.time.logon) +
				  ch->player.time.played, 0);

sprintf(buf + strlen(buf),       "|=|Log[%4d]Days  |                  | Align  [%5d]         |/|\r\n",
playing_time.day, GET_ALIGNMENT(ch));
sprintf(buf + strlen(buf),       "|/|     [%2d]Hours |                  | Race   [Hum] Age[%3d]  |=|\r\n",
playing_time.hours, GET_AGE(ch));
strcat(buf, "|/|-----------------------------------------------------------|=|\r\n");
strcat(buf, "|=| MANA POOL. [Power rate][ - %]                     |  BURN |/|\r\n");
strcat(buf, "|/| `wWHITE [                                             ]MAX`n  |=|\r\n");
strcat(buf, "|=| `cBLUE  [                                             ]MAX`n  |/|\r\n");
strcat(buf, "|=| `gGREEN [    `wPlease Note: Power is not yet added.     `g]MAX`n  |/|\r\n");
strcat(buf, "|=| `rRED   [                                             ]MAX`n  |/|\r\n");
strcat(buf, "|=| `bBLACK [                                             ]MAX`n  |/|\r\n");
strcat(buf, "|=|-----------------------------------------------------------|/|\r\n");
strcat(buf, "(.)                                                           (.)\r\n");
strcat(buf,  "\r\n");
...
ACMD(do_help)
{
  ...
  if (this_help->min_level > GET_LEVEL(ch)) {
    send_to_char("You are not able to find help for that word.\r\n", ch);
    return;
  }
...
/* end while (parser) */
  ...
  strcpy(godbuf, " Listing of the Gods\r\n-=-=-=-=-=-=-=-=-=-=-\r\n");
  strcpy(mortbuf," List of the Mortals\r\n-=-=-=-=-=-=-=-=-=-=-\r\n");
  ...
  if (!PRF_FLAGGED(wch, PRF_ANON)) {
    if (GET_LEVEL(wch) >= LVL_IMMORT) {
      ...
      sprintf(mortbuf, "%s[%2d %s] %s %s", mortbuf, GET_LEVEL(wch),CLASS_ABBR(wch), GET_NAME(wch), GET_TITLE(wch));
      morts++;
    }
  } else {
    if (GET_LEVEL(wch) >= LVL_IMMORT) {
      sprintf(godbuf, "%s[ - Anonymous - ] %s %s", godbuf, GET_NAME(wch), GET_TITLE(wch));
      gods++;
    } else {
      sprintf(mortbuf, "%s[ - Anonymous - ] %s %s", mortbuf, GET_NAME(wch), GET_TITLE(wch));
      morts++;
    }
  }
    *buf = '\0';
    if (GET_INVIS_LEV(wch))
      sprintf(buf, "%s `c(i%d)`n", buf, GET_INVIS_LEV(wch));
...
/* end of for */
  ...
  if (morts == 0)
    sprintf(mortbuf, "%s\r", mortbuf);

    sprintf(mortbuf, "%s\r\nThere %s %d visible mortal%s.", mortbuf,
...
ACMD(do_finger)
{
  send_to_char("This command has not been implemented yet.\r\n", ch);
}
...
ACMD(do_toggle)
{
  ...
  sprintf(buf,
	  ...
	  ONOFF(!PRF_FLAGGED(ch, PRF_NOROLE)),
	  ONOFF(!PRF_FLAGGED(ch, PRF_NOAUCT)),
	  ONOFF(!PRF_FLAGGED(ch, PRF_NOGRATZ)),

	  ONOFF(!PRF_FLAGGED(ch, PRF_NOOOC)),
	  ONOFF(!PRF_FLAGGED(ch, PRF_NONEW)),
